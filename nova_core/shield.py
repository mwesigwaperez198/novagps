...
class NovaDeterministicShield:
    ...
    def _nginx_tls_hardening_source(self) -> str:
        return (
            ...
            "    return findings\n"
            "\n"
            "def nginx_hardening_block(findings):\n"
            "    \"\"\"Dynamically synthesize a production-ready Nginx TLS configuration.\"\"\"\n"
            ...
            "        '}\\n'\n"
            "        '\\n'\n"
            "        '# zero-millisecond reject of weak initiators / unknown SNI (never negotiates)\\n'\n"
            "        'server {\\n'\n"
            ...
            "        '    ssl_certificate_key /etc/nginx/ssl/privkey.pem;\\n'\n"
            "        '\\n'\n"
            "        '    # protocol + cipher floor: anything below dies at the ClientHello\\n'\n"
            ...
            "        '\\n'\n"
            "        '    # forward-secrecy session hardening\\n'\n"
            "        '    ssl_session_cache shared:SSL:10m;\\n'\n"
            "        '    ssl_session_timeout 1d;\\n'\n"
            "        '    ssl_session_tickets off;\\n'\n"
            "        '\\n'\n"
            "        '    # cert transparency + expiry enforcement\\n'\n"
            "        '    ssl_stapling on;\\n'\n"
            ...
            "            print('--- LIVE TLS TELEMETRY (%s) ---' % TARGET)\n"
            "            print(json.dumps(matrix.get('ssl_telemetry', {}), indent=2))\n"
            "        except Exception as probe_fault:\n"
            "            print('live probe rejected:', probe_fault)\n"
            "            matrix = sim_weak()\n"
            "    else:\n"
            "        print('live lane disabled - synthetic weak telemetry injected to exercise the synthesizer')\n"
            "        matrix = sim_weak()\n"
            ...
            "    print('--- DYNAMIC: production-ready Nginx hardening block ---')\n"
            "    print(nginx_hardening_block(findings))\n"
