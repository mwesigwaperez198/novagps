...
COMMAND_REGISTRY: Mapping[str, CommandSpec] = MappingProxyType(
    {
        ...
        "system.disk": CommandSpec(
            command_id="system.disk",
        ...
        "system.memory": CommandSpec(
            ...
            image="alpine:3.20",
            argv=("sh", "-c", "free -m 2>/dev/null || cat /proc/meminfo"),
            allowed_roles=("viewer", "operator", "admin"),
        ),
        ...
        "net.ping": CommandSpec(
            command_id="net.ping",
            description="ICMP ping to a target host (3 packets).",
            image="alpine:3.20",
        ...
        "net.traceroute": CommandSpec(
            command_id="net.traceroute",
            description="Trace network path to a target host.",
            image="alpine:3.20",
            argv=("traceroute", "-m", "15", "{target}"),
            allowed_roles=("operator", "admin"),
            allowed_args=("target",),
            timeout_seconds=30,
        ),
        "net.dns.multi": CommandSpec(
            command_id="net.dns.multi",
            description="Multi-tool DNS resolution of a target hostname.",
            image="alpine:3.20",
            argv=("sh", "-c", "nslookup {target} && echo '---' && cat /etc/resolv.conf"),
            allowed_roles=("auditor", "operator", "admin"),
            allowed_args=("target",),
            timeout_seconds=15,
        ),
        "crypto.verify": CommandSpec(
            command_id="crypto.verify",
            description="Verify a TLS certificate chain.",
            image="alpine:3.20",
            argv=("sh", "-c", "openssl s_client -connect {target}:443 </dev/null 2>/dev/null | openssl x509 -noout -text 2>/dev/null | head -30"),
            allowed_roles=("operator", "admin"),
            allowed_args=("target",),
            timeout_seconds=20,
        ),
